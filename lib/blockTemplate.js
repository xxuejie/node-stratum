...
function createGeneration(rewardType, rpcData, publicKey, extraNoncePlaceHolder) {
    switch (rewardType) {
        case 'POW':
            return transactions.CreateGenerationPOW(//new transactions.Generation(
                rpcData,
                publicKey,
                extraNoncePlaceHolder
            );
        case 'POS':
            return transactions.CreateGenerationPOS(//new transactions.Generation(
                rpcData,
                publicKey,
                extraNoncePlaceHolder
            );
        default:
            console.error("invalid rewardType: "+rewardType);
            process.exit(1);
    }
}
...
**/
var BlockTemplate = module.exports = function BlockTemplate(rewardType, jobId, rpcData, publicKey, extraNoncePlaceholder){
    ...
    this.generationTransaction = createGeneration(
        rewardType,
        rpcData,
        publicKey,
        extraNoncePlaceholder
